...
import {
  ...
  villaSchema, createVillaInputSchema, updateVillaInputSchema, searchVillaInputSchema,
  villaPhotoSchema, createVillaPhotoInputSchema, updateVillaPhotoInputSchema, searchVillaPhotoInputSchema,
  amenitySchema, searchAmenityInputSchema,
  villaAmenitySchema, createVillaAmenityInputSchema,
  villaRuleSchema, createVillaRuleInputSchema,
  villaCalendarSchema, createVillaCalendarInputSchema,
  villaPricingOverrideSchema, createVillaPricingOverrideInputSchema,
  villaWishlistSchema, createVillaWishlistInputSchema,
  wishlistItemSchema, createWishlistItemInputSchema,
  bookingSchema, createBookingInputSchema, updateBookingInputSchema,
  bookingGuestSchema, createBookingGuestInputSchema,
  bookingHistorySchema, createBookingHistoryInputSchema,
  bookingPaymentSchema, createBookingPaymentInputSchema,
  messageThreadSchema, createMessageThreadInputSchema,
  messageSchema, createMessageInputSchema,
  userReviewSchema, createUserReviewInputSchema,
  guestReviewSchema, createGuestReviewInputSchema,
  notificationSchema, createNotificationInputSchema,
  passwordResetSchema, createPasswordResetInputSchema,
} from './schema.ts';
...
const { Pool } = pkg;
const {
  DATABASE_URL, PGHOST, PGDATABASE, PGUSER, PGPASSWORD, PGPORT = 5432,
...
io.on('connection', (socket) => {
  ...
  socket.on('disconnect', () => {
    socketsByUserId[user_id] = (socketsByUserId[user_id] || []).filter(s => s.id !== socket.id);
  });
});
...
async function emitToThreadParticipants(thread_id, event, payload) {
  ...
  try {
    ...
    const { rows } = await client.query(
      `SELECT participant_user_id FROM message_threads WHERE thread_id=$1`, [thread_id]
    );
 ...
 * User Signup
 */
app.post('/auth/signup', asyncWrap(async (req, res) => {
  /*
  ...
  */
  ...
  try {
    ...
    const exists = await client.query(`SELECT 1 FROM users WHERE email = $1`, [body.email]);
    ...
    const now = nowIso();
    const newUser = {
      user_id, email: body.email, password_hash,
      display_name: body.display_name,
      profile_photo_url: `https://picsum.photos/seed/${user_id}/200`,
      bio: null, contact_email: body.email,
      is_host: false, is_superhost: false,
      last_active_at: now,
      created_at: now,
      updated_at: now
    };
    ...
    await client.query(
      `INSERT INTO users (${Object.keys(validUser).join(',')})
       VALUES (${Object.keys(validUser).map((_,i)=>'$'+(i+1)).join(',')})`,
...
app.post('/auth/login', asyncWrap(async (req, res) => {
  ...
  */
  ...
  const client = await pool.connect();
  try {
    const { rows } = await client.query(`SELECT * FROM users WHERE email = $1`, [body.email]);
 ...
 */
app.post('/auth/password/request-reset', asyncWrap(async (req, res) => {
  /*
  ...
  */
  ...
  const [body, err] = safeParse(schema, req.body);
  if (err) return res.status(200).json({ message: 'If registered, a reset email sent.' });
  const client = await pool.connect();
  try {
    const { rows } = await client.query('SELECT user_id FROM users WHERE email = $1', [body.email]);
    if (!rows.length) return res.status(200).json({ message: 'If registered, a reset email sent.' });
 ...
 */
app.post('/auth/password/reset', asyncWrap(async (req, res) => {
  /*
  ...
  */
  ...
  try {
    const { rows } = await client.query(
      `SELECT * FROM password_resets WHERE reset_code=$1 ORDER BY created_at DESC LIMIT 1`, [body.reset_code]
    );
    if (!rows.length) return res.status(400).json({ error: 'Invalid or expired code' });
    const pr = rows[0];
    if (pr.used) return res.status(400).json({ error: 'Reset code already used' });
    if (new Date(pr.expires_at) < new Date()) return res.status(400).json({ error: 'Reset code expired' });
    // Update password
    ...
    await client.query(`UPDATE password_resets SET used=TRUE WHERE password_reset_id=$1`, [pr.password_reset_id]);
 ...
 */
app.get('/account/me', requireAuth, asyncWrap(async (req, res) => {
  const client = await pool.connect();
  try {
    const { rows } = await client.query(
      `SELECT * FROM users WHERE user_id=$1`, [req.user.user_id]
    );
 ...
 */
app.patch('/account/me', requireAuth, asyncWrap(async (req, res) => {
  const allowed = ['display_name', 'profile_photo_url', 'bio', 'contact_email'];
  const updates = {};
  for (const k of allowed) if (k in req.body) updates[k] = req.body[k];
  if (Object.keys(updates).length === 0)
    return res.status(400).json({ error: 'No updatable fields provided' });

  const [valid, err] = safeParse(updateUserInputSchema, { user_id: req.user.user_id, ...updates });
  if (err) return res.status(400).json({ error: err.message });
  ...
  try {
    ...
    const { rows } = await client.query(`SELECT * FROM users WHERE user_id=$1`, [req.user.user_id]);
...
function buildVillaSearchSQL(q) {
  ...
  let params = [];
  let i = 1;
  if(q.location) { sql += ` AND (address_city ILIKE $${i} OR address_area ILIKE $${i})`; params.push('%'+q.location+'%'); i++; }
  if(q.price_min!=null) { sql += ` AND price_per_night >= $${i}`; params.push(q.price_min); i++; }
  if(q.price_max!=null) { sql += ` AND price_per_night <= $${i}`; params.push(q.price_max); i++; }
  if(q.bedrooms!=null) { sql += ` AND bedrooms >= $${i}`; params.push(q.bedrooms); i++; }
  if(q.beds!=null) { sql += ` AND beds >= $${i}`; params.push(q.beds); i++; }
  if(q.bathrooms!=null) { sql += ` AND bathrooms >= $${i}`; params.push(q.bathrooms); i++; }
  if(q.is_beachfront!=null) { sql += ` AND is_beachfront = $${i}`; params.push(q.is_beachfront); i++; }
  if(q.is_pet_friendly!=null) { sql += ` AND is_pet_friendly = $${i}`; params.push(q.is_pet_friendly); i++; }
  if(q.is_instant_book!=null) { sql += ` AND is_instant_book = $${i}`; params.push(q.is_instant_book); i++; }
  if(q.num_guests!=null) { sql += ` AND max_guests >= $${i}`; params.push(q.num_guests); i++; }
  ...
  // Sorting
  switch(q.sort) {
    case 'price_low_high': orderSql = 'ORDER BY price_per_night ASC'; break;
    case 'price_high_low': orderSql = 'ORDER BY price_per_night DESC'; break;
    case 'rating': orderSql = 'ORDER BY avg_rating DESC'; break;
    case 'newest': orderSql = 'ORDER BY v.created_at DESC'; break;
    case 'popularity': orderSql = 'ORDER BY reviews_count DESC'; break;
    default: orderSql = 'ORDER BY v.created_at DESC';
  }
  // Pagination
 ...
 */
app.get('/villas', asyncWrap(async (req,res) => {
  /*
 ...
 */
app.get('/villas/featured', asyncWrap(async (req, res) => {
  /*
 ...
 */
app.get('/villas/popular-locations', asyncWrap(async (req, res) => {
  /*
 ...
 * Villa Details [GET]
 */
app.get('/villas/:villa_id', asyncWrap(async (req, res) => {
  /*
  ...
  */
  ...
  try {
    const { rows } = await client.query(
      `SELECT * FROM villas WHERE villa_id = $1`, [villa_id]
    );
    if (!rows.length) return res.status(404).json({ error: 'Villa not found' });
    const villa = rows[0];
 ...
 * Villa Details [PATCH]
 */
app.patch('/villas/:villa_id', requireAuth, asyncWrap(async (req, res) => {
  /*
  ...
  */
  ...
  try {
    ...
    // PATCH fields filter
    const body = Object.fromEntries(Object.entries(req.body).filter(([k,_]) =>
      Object.hasOwn(villa, k)
    ));
    ...
    const updateCols = Object.keys(body)
      .filter(k => k !== 'villa_id')
      .map((k,i)=> `${k} = $${i+2}`);
    const updateVals = Object.values(body).filter((_,i)=>Object.keys(body)[i]!=='villa_id');
    await client.query(
 ...
 * Villa Photos [GET]
 */
app.get('/villas/:villa_id/photos', asyncWrap(async (req, res) => {
  /*
  ...
  */
  ...
  try {
    const { rows } = await client.query(
      `SELECT * FROM villa_photos WHERE villa_id=$1 ORDER BY ordering ASC`, [villa_id]);
 ...
 * Villa Amenities [GET]
 */
app.get('/villas/:villa_id/amenities', asyncWrap(async (req, res) => {
  /*
 ...
 * Villa Rules [GET]
 */
app.get('/villas/:villa_id/rules', asyncWrap(async (req, res) => {
  /*
  ...
  */
  ...
  try {
    const { rows } = await client.query(
      `SELECT * FROM villa_rules WHERE villa_id=$1 ORDER BY rule_type ASC`, [villa_id]);
    return res.json(rows.map(row => ({
 ...
 * Villa Calendar [GET]
 */
app.get('/villas/:villa_id/calendar', asyncWrap(async (req, res) => {
  /*
 ...
 */
app.patch('/villa/:villa_id/calendar', requireAuth, asyncWrap(async (req, res) => {
  /*
  ...
  */
  ...
  try {
    ...
    const { rows: villas } = await client.query(`SELECT * FROM villas WHERE villa_id=$1`, [villa_id]);
    ...
    for (const d of req.body) {
      ...
      await client.query(
        ...
        [vcd.villa_calendar_id, vcd.villa_id, vcd.date, vcd.is_available, vcd.reason, vcd.updated_at]
      );
    }
    // Return updated days
    const { rows } = await client.query(
 ...
 */
app.get('/villas/:villa_id/pricing-overrides', asyncWrap(async (req, res) => {
  const villa_id = req.params.villa_id;
  const client = await pool.connect();
  try {
    const { rows } = await client.query(
 ...
 */
app.patch('/villa/:villa_id/pricing-overrides', requireAuth, asyncWrap(async (req, res) => {
  /*
  ...
  */
  ...
  try {
    // Host permission
    const { rows } = await client.query(`SELECT owner_user_id FROM villas WHERE villa_id=$1`, [villa_id]);
    ...
    for (const p of req.body) {
      ...
      await client.query(
        ...
        [vo_id, villa_id, p.date, p.nightly_price, p.min_nights, p.max_nights, nowIso()]
      );
    }
    // Fetch return
 ...
 */
app.get('/villas/amenities', asyncWrap(async (_req, res) => {
  /*
 ...
 * Host Villas [GET]
 */
app.get('/host/villas', requireAuth, asyncWrap(async (req, res) => {
  const client = await pool.connect();
  try {
    const { rows } = await client.query(
      `SELECT v.*, COALESCE(AVG(r.rating), 0.0) as avg_rating, COUNT(r.review_id) as reviews_count
 ...
 */
app.post('/host/villas', requireAuth, asyncWrap(async (req, res) => {
  /*
  ...
  */
  ...
  const body = { ...rest, owner_user_id: req.user.user_id, villa_id: genId('v'), created_at: nowIso(), updated_at: nowIso() };
  ...
  try {
    ...
    await client.query(
      `INSERT INTO villas (${Object.keys(valid).join(',')})
       VALUES (${Object.keys(valid).map((_,i)=>'$'+(i+1)).join(',')})`,
      Object.values(valid)
    );
      ...
      for (const [i, photo] of req.body.photos.entries()) {
        await client.query(
          `INSERT INTO villa_photos (villa_photo_id, villa_id, photo_url, ordering, is_main, uploaded_at)
           VALUES ($1,$2,$3,$4,$5,$6)`,
          [genId('vp'), body.villa_id, photo.photo_url, i+1, photo.is_main||false, nowIso()]
        );
      }
    }
      ...
      for (const rule of req.body.rules) {
        await client.query(
          `INSERT INTO villa_rules (villa_rule_id, villa_id, rule_type, allowed, notes)
           VALUES ($1,$2,$3,$4,$5)`,
           [genId('vr'), body.villa_id, rule.rule_type, rule.allowed, rule.notes||null]
        );
      }
    }
    await client.query('COMMIT');
    const { rows } = await client.query(`SELECT * FROM villas WHERE villa_id=$1`, [body.villa_id]);
 ...
 */
async function isVillaAvailable(villa_id, checkin_str, checkout_str, num_guests) {
  const client = await pool.connect();
  try {
    ...
    if (num_guests) {
      const villaRow = await client.query(
        `SELECT max_guests FROM villas WHERE villa_id=$1`, [villa_id]);
...
async function computeBookingPrice(villa_id, checkin_str, checkout_str) {
  ...
  try {
    ...
    const villa = rows[0];
    const nights = (new Date(checkout_str) - new Date(checkin_str))/(1000*60*60*24);
 ...
 */
app.post('/villas/:villa_id/booking/preview', asyncWrap(async (req, res) => {
  /*
  ...
  */
  ...
  const available = await isVillaAvailable(villa_id, checkin_date, checkout_date, num_guests);
  if (!available)
    return res.json({ available: false });
  const price_summary = await computeBookingPrice(villa_id, checkin_date, checkout_date);
 ...
 * Create Booking [POST]
 */
app.post('/villas/:villa_id/booking', requireAuth, asyncWrap(async (req, res) => {
  /*
  ...
  */
  ...
  try {
    const { rows } = await client.query(`SELECT * FROM villas WHERE villa_id=$1`, [villa_id]);
    if (!rows.length) return res.status(400).json({ error: 'Villa does not exist' });
    const villa = rows[0];
    // Check dates available
    const available = await isVillaAvailable(villa_id, body.checkin_date, body.checkout_date, body.num_guests);
    if (!available) return res.status(400).json({ error: 'Selected dates unavailable' });
    // Compute price
    const price = await computeBookingPrice(villa_id, body.checkin_date, body.checkout_date);
    ...
    await client.query(
      `INSERT INTO bookings (booking_id, villa_id, guest_user_id, host_user_id, checkin_date, checkout_date, num_guests, status, is_instant_book,
       ...
       [
         booking_id, villa_id, req.user.user_id, villa.owner_user_id,
         body.checkin_date, body.checkout_date, body.num_guests, status, is_instant,
         price.nightly_price, price.cleaning_fee, price.service_fee, price.taxes, price.total_price,
         'paid', now, now
       ]
    );
    ...
    await client.query(
      `INSERT INTO booking_guests (booking_guest_id, booking_id, guest_name, guest_email, special_requests)
       VALUES ($1, $2, $3, $4, $5)`,
       [genId('bg'), booking_id, body.guest_details.name, body.guest_details.email, body.guest_details.special_requests||null]
    );
    // Booking history
    await client.query(
      `INSERT INTO booking_histories (booking_history_id, booking_id, action, previous_value, new_value, action_by_user_id, action_at)
       VALUES ($1,$2,$3,$4,$5,$6,$7)`,
      [genId('bh'), booking_id, status, null, JSON.stringify({ status }), req.user.user_id, now]
    );
    ...
    // Respond
    const { rows: ret } = await client.query(`SELECT * FROM bookings WHERE booking_id=$1`, [booking_id]);
    const out = { ...ret[0] };
    return res.json(out);
  } finally { client.release(); }
}));
...
app.get('/villas/:villa_id/reviews', asyncWrap(async (req, res) => {
