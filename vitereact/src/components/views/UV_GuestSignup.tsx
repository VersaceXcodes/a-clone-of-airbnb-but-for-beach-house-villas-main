...
const emailSchema = z.string().email("Please enter a valid email").max(255, "Email too long");
const passwordSchema = z.string().min(8, "Password must be at least 8 characters").max(255, "Password too long");
const displayNameSchema = z.string().min(1, "Display name is required").max(255, "Display name is too long");
...
const UV_GuestSignup: React.FC = () => {
  ...
  const [profile_photo_url, set_profile_photo_url] = useState<string | null>(null); // for preview only
  ...
  const validateFields = () => {
    const fieldErrors: { email?: string; password?: string; display_name?: string; agree_terms?: string } = {};
    try {
  ...
  const validation = useMemo(validateFields, [email, password, display_name, agree_terms]);
    ...
    content = (
      <div className="flex flex-col items-center justify-center w-full min-h-[70vh] py-10 bg-gray-50">
        <form
          ...
          noValidate
        >
          <h2 className="text-2xl md:text-3xl font-bold text-center mb-6">Create your BeachStay account</h2>
          ...
          <div className="flex flex-col items-center mb-6">
            ...
            <span className="text-xs text-gray-400 mt-1">(Optional, not shared until booking)</span>
          </div>

          {/* Display name */}
          <div className="mb-4">
            <label htmlFor="display_name" className="block text-sm font-semibold mb-1">
              Display name
            </label>
            <input
              id="display_name"
              ...
              aria-describedby={validation.display_name ? "display_name_error" : undefined}
            />
            {validation.display_name && (
              <span className="text-xs text-red-500" id="display_name_error">{validation.display_name}</span>
            )}
          </div>
          {/* Email */}
          <div className="mb-4">
            ...
            />
            {validation.email && (
              <span className="text-xs text-red-500" id="email_error">{validation.email}</span>
            )}
          </div>
          {/* Password */}
          <div className="mb-4">
            ...
            <div className="relative">
              ...
              >
                {show_password ? (
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M13.875 18.825A10.05 10.05 0 0112 19c-5 0-9-4.477-9-7s4-7 9-7c1.262 0 2.466.247 3.572.693M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                ) : (
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M3 3l18 18M9.88 9.88a3 3 0 104.24 4.24m2.06-2.06c.912-.912 1.832-2.181 1.83-4.06C17 7 13 3 12 3s-5 4-5 7c0 2.274.772 3.572 1.47 4.27m.53.53c.878.878 2.153 1.853 4.06 1.83" /></svg>
                )}
              </button>
            </div>
            {validation.password && (
              <span className="text-xs text-red-500" id="password_error">{validation.password}</span>
            )}
          </div>
          ...
          <div className="mb-4">
            ...
            {validation.agree_terms && (
              <div className="text-xs text-red-500 mt-1" id="terms_error">{validation.agree_terms}</div>
            )}
          </div>
