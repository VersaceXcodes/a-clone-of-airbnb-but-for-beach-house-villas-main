...
const UV_HostSignup: React.FC = () => {
	...
	const isExistingUser = user_session.is_authenticated;

	// Form state
	const [email, set_email] = useState<string>(user_session.email || "");
	...
	// Redirect if user is already a host
	useEffect(() => {
	...
	const hostSignupMutation = useMutation({
		...
		onSuccess: (data) => {
			set_user_session({
				...
				profile_photo_url:
					typeof data.profile_photo_url === "string" && data.profile_photo_url
						? data.profile_photo_url
						: "",
				superhost_status:
					typeof data.superhost_status === "boolean" ? data.superhost_status : false,
	...
	// ---- Mutation for upgrading existing user to host ----
	const upgradeToHostMutation = useMutation({
		mutationFn: async () => {
			const res = await axios.patch(
				`${API_BASE_URL}/user/upgrade-to-host`,
				{},
				{
					headers: {
						"Content-Type": "application/json",
		...
		onError: (err: any) => {
			...
			if (axios.isAxiosError(err) && err.response?.data?.error) {
				...
				set_error("Could not upgrade to host. Please try again.");
			}
		},
	});
	...
	const handleSubmit: React.FormEventHandler<HTMLFormElement> = (e) => {
		...
		if (isExistingUser) {
			// For existing users, just upgrade to host
			...
			set_loading(true);
			upgradeToHostMutation.mutate();
		} else {
			// For new users, validate all fields and create account
			if (!email.trim()) {
			...
			<div className="flex w-full min-h-[calc(100vh-8rem)] items-center justify-center bg-gray-50 py-10 px-4">
				<div className="w-full max-w-md bg-white rounded-xl shadow-md p-8 space-y-7 border border-gray-100">
					<h1 className="text-2xl font-bold text-center text-gray-700">
						{isExistingUser ? "Upgrade to Host" : "Become a BeachStay Host"}
					</h1>
					<p className="text-center text-gray-500 text-sm mb-4">
						{isExistingUser ? (
							<>
								Welcome back, {user_session.display_name}! Upgrade your account to start
								listing your villa rentals and earning.
							</>
						) : (
							<>
								Create your host account to list your villa rentals and start earning.
								Already have an account?{" "}
								<Link
									className="text-blue-600 underline hover:text-blue-800"
									to="/guest/login?returnTo=/host/signup"
								>
									Log in
								</Link>
					...
					<form className="space-y-5" onSubmit={handleSubmit} noValidate>
						...
						<div>
							...
							<input
								...
								className={`w-full rounded border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-100 ${isExistingUser ? "bg-gray-100" : ""}`}
								disabled={loading || isExistingUser}
								required
							/>
							{isExistingUser && (
								<p className="text-xs text-gray-400 mt-1">
									Your existing display name will be used.
								</p>
							)}
						</div>
						...
						<div>
							...
							<div className="flex items-center mt-2 space-x-2">
								<img
									src={
										imgPreviewError || !profile_photo_url
						...
						<div>
							<button
								...
								disabled={
									loading ||
									(!isExistingUser && (!email || !password || !display_name)) ||
									!agree_to_host_tos
								}
							>
								{loading ? (
