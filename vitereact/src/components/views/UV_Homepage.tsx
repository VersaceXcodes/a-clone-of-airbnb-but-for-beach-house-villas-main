...
const fetchFeaturedVillas = async (): Promise<VillaSummary[]> => {
  ...
  // Map ids to string, and main_photo_url fallback as described
...
const UV_Homepage: React.FC = () => {
  ...
  const personalized_greeting = user_session.is_authenticated && user_session.display_name
  ...
  const onSubmitSearch = useCallback(
    (e: React.FormEvent<HTMLFormElement>) => {
      ...
      if (num_guests && num_guests > 0) params.append("num_guests", String(num_guests));
      navigate(`/search?${params.toString()}`);
    },
    [location, checkin_date, checkout_date, num_guests, set_search_state, navigate, search_state]
  );
  ...
  return (
    <>
      {/* Hero Section */}
      <div className="relative w-full bg-cover bg-center min-h-[350px] flex items-center justify-center" style={{ backgroundImage: "url('https://picsum.photos/seed/beachhero/1600/700')" }}>
        ...
        <div className="z-10 flex flex-col items-center w-full max-w-5xl mx-auto px-4 py-16">
          ...
          >
            ...
            <input
              ...
              min={1}
              value={num_guests}
              onChange={e => set_num_guests(Math.max(1, parseInt(e.target.value) || 1))}
              placeholder="Guests"
            />
            <button
              type="submit"
      ...
      <div className="w-full max-w-7xl mx-auto px-4 md:px-8 py-12 flex flex-col gap-16">
                    ...
                    >
                      ...
                      <div className="flex-1 flex flex-col px-4 pt-3 pb-4">
                        ...
                        <div className="flex flex-row items-center text-sm text-gray-500 gap-2 mt-0.5">
                          <svg className="w-4 h-4 text-blue-400 inline" fill="currentColor" viewBox="0 0 20 20"><path d="M10 18c-6-5.39-8-7.182-8-10A8 8 0 118 18h4zm0-8a2 2 0 110-4 2 2 0 010 4z"/></svg>
        ...
        <div className="mx-auto max-w-3xl text-center text-lg text-gray-600 py-3">
          <span>
            Discover your new favorite beach <span className="font-semibold text-blue-500">getaway</span> – from sun-kissed coasts to secret escapes, BeachStay Villas brings the ocean closer to you.
          </span>
        </div>
      </div>
    </>
