...
const UV_HostOnboardingWizard: React.FC = () => {
	...
	useEffect(() => {
		if (!user_session.is_authenticated) {
			navigate("/guest/login?returnTo=/host/onboarding"); // redirect to login with return URL
		} else if (user_session.is_authenticated && !user_session.is_host) {
			navigate("/host/signup"); // redirect non-hosts to host signup
		}
						...
						<div>
							...
							{is_edit_mode && (
								<div className="mt-4 text-yellow-700 bg-yellow-50 p-2 rounded border border-yellow-200 text-sm">
									...
									listings. Photo management for existing villas is not supported in this
									MVP.
								</div>
							)}
						</div>
