...
const UV_HostOnboardingWizard: React.FC = () => {
  ...
  const parsedSlugVillaId = slugVillaId && !isNaN(Number(slugVillaId)) ? parseInt(slugVillaId, 10) : null;
  const initialVillaId: number | null = parsedSlugVillaId;
  const initialUserId: number | null = user_session.user_id !== null && !isNaN(Number(user_session.user_id)) ? Number(user_session.user_id) : null;
  ...
  const { data: masterAmenities, isFetching: amenitiesLoading, error: amenitiesError } = useQuery<Amenity[], Error>({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        // Main villa object
        const res_villa = await axios.get(`${API_BASE}/villa/${parsedSlugVillaId}`);
        const villa: Villa = villaSchema.parse(res_villa.data);

        // Photos
        const res_photos = await axios.get(`${API_BASE}/villa/${parsedSlugVillaId}/photos`);
        const photos: VillaPhoto[] = z.array(villaPhotoSchema).parse(res_photos.data);
        ...
        const res_amenities = await axios.get(`${API_BASE}/villa/${parsedSlugVillaId}/amenities`);
        const villaAmenities: Amenity[] = z.array(amenitySchema).parse(res_amenities.data);
        const amenity_ids = villaAmenities.map((a) => Number(a.amenity_id));

        // Rules
        const res_rules = await axios.get(`${API_BASE}/villa/${parsedSlugVillaId}/rules`);
        ...
        // Calendar
        const res_cal = await axios.get(`${API_BASE}/villa/${parsedSlugVillaId}/calendar`);
        const calendar: VillaCalendar[] = z.array(villaCalendarSchema).parse(res_cal.data);
        ...
        setWizard((prev) => ({
          ...
          main_photo_url: villa.main_photo_url || (wizardPhotos.find((p) => p.is_main)?.photo_url ?? ""),
          photos: wizardPhotos,
          amenity_ids,
          rules: wizardRules,
        ...
        setWizard((prev) => ({
          ...prev,
          loading: false,
          error: e?.response?.data?.error || "Failed to load villa data.",
        }));
      }
    })();
  ...
  function isStepValid(idx: WizardStep, w: WizardState): boolean {
    switch (idx) {
      ...
      case 1:
        return w.photos.length >= 5 && w.photos.length <= 20 && w.photos.some((p) => p.is_main);
      case 2:
  ...
  const createVillaMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      setWizard((prev) => ({ ...prev, villa_id: Number(data.villa_id), loading: false }));
    ...
    onError: (err: any) => {
      setWizard((prev) => ({
        ...prev,
        loading: false,
        error: err?.response?.data?.error || "Failed to create listing.",
      }));
    },
  });
  ...
  const editVillaMutation = useMutation({
    mutationFn: async (payload: any) => {
      ...
      const res = await axios.patch(`${API_BASE}/villa/${wizard.villa_id}`, fullBody, {
        headers: { Authorization: `Bearer ${user_session.token || ""}` },
      });
      return villaSchema.parse(res.data);
    },
  ...
  const updateCalendarMutation = useMutation({
    mutationFn: async (days: any[]) => {
      ...
      const res = await axios.patch(`${API_BASE}/villa/${wizard.villa_id}/calendar`, days, {
        headers: {
  ...
  function isInputDisabledOnEdit(): boolean { return is_edit_mode; }
  ...
  function addPhotoUrl(url: string) {
    ...
    setWizard((prev) => {
      ...
      return {
        ...
        main_photo_url: prev.main_photo_url || (prev.photos.length === 0 ? url : prev.main_photo_url),
        error: null,
      };
    });
  }
  ...
  useEffect(() => {
    if (wizard.step_index === 5 && !is_edit_mode && wizard.calendar_data.length === 0) {
      // Next 60 days, all available
      const days: { date: string; is_available: boolean; reason: string | null }[] = [];
  ...
  // --- Redirect unauthenticated/non-host
  useEffect(() => {
    if (!user_session.is_authenticated || !user_session.is_host) {
      navigate("/guest/login"); // must be logged-in host
    }
  ...
  return (
    <>
      <div className="max-w-3xl mx-auto py-8 px-4">
            ...
            {STEP_LABELS.map((label, i) => (
              <div key={label} className="flex-1">
                <div className="flex items-center">
                  ...
                  <span className={(i === wizard.step_index ? "text-blue-700" : "text-gray-500") + " whitespace-nowrap"}>
                    {label}
                  </span>
                </div>
                {i < TOTAL_STEPS - 1 && (
        ...
        <div className="mt-2 mb-8">
          {wizard.step_index === 0 && (
            <div>
              ...
              <div className="grid grid-cols-1 gap-3">
                <div>
                  <label className="font-semibold" htmlFor="wizard_title">Title*</label>
                ...
                <div>
                  <label className="font-semibold" htmlFor="wizard_desc_short">Short Description*</label>
                  <textarea className="input input-bordered w-full mt-1"
                ...
                <div>
                  <label className="font-semibold" htmlFor="wizard_desc_long">Detailed Description*</label>
                  <textarea className="input input-bordered w-full mt-1"
                    id="wizard_desc_long"
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  <div>
                    <label className="font-semibold" htmlFor="wizard_street">Street</label>
                    <input className="input input-bordered w-full mt-1"
                      id="wizard_street"
                  ...
                  <div>
                    <label className="font-semibold" htmlFor="wizard_city">City*</label>
                    <input className="input input-bordered w-full mt-1"
                      id="wizard_city"
                  ...
                  <div>
                    <label className="font-semibold" htmlFor="wizard_area">Area/Neighborhood</label>
                    <input className="input input-bordered w-full mt-1"
                      id="wizard_area"
                  ...
                  <div>
                    <label className="font-semibold" htmlFor="wizard_postal">Postal</label>
                    <input className="input input-bordered w-full mt-1"
                      id="wizard_postal"
                  ...
                  <div>
                    <label className="font-semibold" htmlFor="wizard_country">Country*</label>
                    <input className="input input-bordered w-full mt-1"
                      id="wizard_country"
                  ...
                  <div>
                    <label className="font-semibold" htmlFor="wizard_lat">Latitude</label>
                    <input
                  ...
                  <div>
                    <label className="font-semibold" htmlFor="wizard_lng">Longitude</label>
                    <input
                      className="input input-bordered w-full mt-1"
                      type="number"
                      id="wizard_lng"
          ...
          {wizard.step_index === 1 && (
            <div>
              ...
              <div className="mb-3">
                ...
                <div className="flex flex-col md:flex-row gap-4 mt-2">
                  <input
                    ...
                    onKeyDown={e => {
                      if (e.key === "Enter" && e.currentTarget.value.trim() && !is_edit_mode) {
              ...
              <div>
                <div className="flex flex-wrap gap-4">
                  {wizard.photos.map((p, idx) => (
                    <div key={p.villa_photo_id} className="relative group">
                      <img src={p.photo_url} alt="Villa" className={`w-40 h-28 object-cover rounded border-2 ${p.is_main ? "border-blue-500" : "border-gray-300"}`} />
                      <button
                ...
                {wizard.photos.length < 5 && <div className="mt-2 text-sm text-red-600">Add at least 5 photos (URLs)</div>}
              </div>
              {is_edit_mode &&
                <div className="mt-4 text-yellow-700 bg-yellow-50 p-2 rounded border border-yellow-200 text-sm">
                  <b>Note:</b> You may only change the photo order/main photo on new listings. Photo management for existing villas is not supported in this MVP.
                </div>
              }
            </div>
          )}
          ...
          {wizard.step_index === 2 && (
            <div>
              ...
              {amenitiesError && <div className="text-red-700 mb-2">{String(amenitiesError)}</div>}
              <div className="flex flex-wrap gap-2">
                {wizard.amenities_options.map((a) => {
                  const checkboxId = `amenity_id_checkbox_${a.amenity_id}`;
                  return (
                    <label key={a.amenity_id} htmlFor={checkboxId} className={"inline-flex items-center border rounded px-3 py-2 cursor-pointer " + (wizard.amenity_ids.includes(a.amenity_id) ? "bg-blue-100 border-blue-400" : "border-gray-300") + (is_edit_mode ? " opacity-50 pointer-events-none" : "") }>
                      <input
                        type="checkbox"
                        id={checkboxId}
                        className="mr-2"
                        ...
                        aria-label={a.label}
                      />
                      {a.icon_url && <img src={a.icon_url} alt="" className="w-5 h-5 mr-1" />}
                      {a.label}
                    </label>
                  );
                })}
              ...
              {is_edit_mode &&
                <div className="mt-4 text-yellow-700 bg-yellow-50 p-2 rounded border border-yellow-200 text-sm">
                  <b>Note:</b> You may only set amenities on new listings. Updating amenities is not supported for existing villas in this MVP.
                </div>
              }
            </div>
          )}
          ...
          {wizard.step_index === 3 && (
            <div>
              ...
              <div>
                <label className="font-semibold" htmlFor="cancellation_policy">Cancellation Policy*</label>
                <select
                  className="input input-bordered w-full mt-1"
              ...
              <div className="mt-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {wizard.rules.map((r, idx) => {
                    ...
                    return (
                      <div key={r.rule_type + idx} className="flex items-center justify-between border rounded px-2 py-1">
                        <div>
                          <span className="capitalize font-medium">{r.rule_type.replace(/_/g, " ")}</span>
                          <input
                            type="checkbox"
                            id={cbId}
                            className="ml-2"
              ...
              <div className="mt-3">
                <label className="font-semibold" htmlFor="additional_house_rules">Additional House Rules</label>
                <textarea
              ...
              {is_edit_mode &&
                <div className="mt-4 text-yellow-700 bg-yellow-50 p-2 rounded border border-yellow-200 text-sm">
                  <b>Note:</b> You may only set rules on new listings. Updating rules is not supported for existing villas in this MVP.
                </div>
              }
            </div>
          )}
          ...
          {wizard.step_index === 4 && (
            <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="font-semibold" htmlFor="field_nightly_rate">Nightly Rate*</label>
                  <input
                    className="input input-bordered w-full"
                    type="number"
                    min={0}
                    ...
                    onChange={e => inputChange("price_per_night", Number(e.target.value))}
                    placeholder="e.g. 320"
                  />
                </div>
                <div>
                  <label className="font-semibold" htmlFor="field_cleaning_fee">Cleaning Fee*</label>
                  <input
                    className="input input-bordered w-full"
                    type="number"
                    min={0}
                ...
                <div>
                  <label className="font-semibold" htmlFor="field_service_fee">Service Fee*</label>
                  <input
                    className="input input-bordered w-full"
                    type="number"
                    min={0}
                ...
                <div>
                  <label className="font-semibold" htmlFor="field_taxes">Taxes*</label>
                  <input
                    className="input input-bordered w-full"
                    type="number"
                    min={0}
                ...
                <div>
                  <label className="font-semibold" htmlFor="field_min_nights">Min Nights*</label>
                  <input
                    className="input input-bordered w-full"
                    type="number"
                    min={1}
                ...
                <div>
                  <label className="font-semibold" htmlFor="field_max_nights">Max Nights*</label>
                  <input
                    className="input input-bordered w-full"
                    type="number"
                ...
                <div>
                  <label className="font-semibold" htmlFor="field_bedrooms">Bedrooms*</label>
                  <input
                    className="input input-bordered w-full"
                    type="number"
                    min={1}
                ...
                <div>
                  <label className="font-semibold" htmlFor="field_beds">Beds*</label>
                  <input
                    className="input input-bordered w-full"
                    type="number"
                    min={1}
                ...
                <div>
                  <label className="font-semibold" htmlFor="field_bathrooms">Bathrooms*</label>
                  <input
                    className="input input-bordered w-full"
                    type="number"
                    min={1}
                ...
                <div>
                  <label className="font-semibold" htmlFor="field_max_guests">Max Guests*</label>
                  <input
                    className="input input-bordered w-full"
                    type="number"
                    min={1}
                ...
                <div className="flex flex-col gap-1">
                  <label className="font-semibold" htmlFor="field_active">Listing Active?</label>
                  <input
                    type="checkbox"
                    id="field_active"
                    className="mr-2"
                ...
                <div className="flex flex-col gap-1">
                  <label className="font-semibold" htmlFor="field_beachfront">Beachfront?</label>
                  <input
                    type="checkbox"
                    id="field_beachfront"
                    className="mr-2"
                ...
                <div className="flex flex-col gap-1">
                  <label className="font-semibold" htmlFor="field_pet">Pet Friendly?</label>
                  <input
                    type="checkbox"
                    id="field_pet"
                    className="mr-2"
                    checked={wizard.is_pet_friendly}
                    onChange={() => inputChange("is_pet_friendly", !wizard.is_pet_friendly)}
                  /> <span className="ml-1">Pet Friendly</span>
                </div>
                <div className="flex flex-col gap-1">
                  <label className="font-semibold" htmlFor="field_instant">Instant Book?</label>
                  <input
                    type="checkbox"
                    id="field_instant"
                    className="mr-2"
                    checked={wizard.is_instant_book}
                    onChange={() => inputChange("is_instant_book", !wizard.is_instant_book)}
          ...
          {wizard.step_index === 5 && (
            <div>
              ...
              <div className="flex flex-col gap-2">
                <span>
                  Toggle days to block (unavailable for guests). Add notes for special restrictions.
                </span>
                <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 gap-2 bg-gray-50 p-3 max-h-[320px] overflow-auto rounded">
                  {wizard.calendar_data.map((d, idx) => {
                    ...
                    return (
                      ...
                      >
                        <div>
                          <label className="font-semibold text-xs" htmlFor={cbId}>{d.date}</label>
                          <input
                            type="checkbox"
                            id={cbId}
                          ...
                          />
                          <span className="text-xs ml-1">{d.is_available ? "Open" : "Blocked"}</span>
                        </div>
                        {!d.is_available && (
                          <input
                            type="text"
                ...
                <div className="flex gap-4 items-start">
                  <div>
                    {wizard.main_photo_url ? (
                      <img src={wizard.main_photo_url} className="rounded-md w-60 h-40 object-cover shadow" alt="Main photo" />
                    ) : (
                      <div className="w-60 h-40 bg-gray-200 rounded flex items-center justify-center text-gray-400">No main photo</div>
                    )}
                  </div>
                  <div className="flex-1">
                    <p className="mb-1"><b>{wizard.address_street && <>{wizard.address_street}, </>}{wizard.address_city}, {wizard.address_country}</b></p>
                    <div className="mb-1 text-sm">Price/Night: <b>${wizard.price_per_night}</b></div>
                    <div className="mb-1 text-sm">Beds: {wizard.beds} &middot; Baths: {wizard.bathrooms} &middot; Max guests: {wizard.max_guests}</div>
                    <div className="mb-1 text-sm">Cancellation: <span className="capitalize">{wizard.cancellation_policy}</span></div>
                    <div className="flex flex-wrap gap-1 mt-2">
                      {wizard.amenities_options.filter(a => wizard.amenity_ids.includes(a.amenity_id)).map((a) =>
                        <span key={a.amenity_id} className="text-xs bg-blue-100 px-2 py-1 rounded">{a.label}</span>
                      )}
                    </div>
                  </div>
                </div>
                <div className="mt-3 text-base">{wizard.description_short}</div>
                <div className="mt-1 text-sm text-gray-600">{wizard.description_long}</div>
                <div className="mt-3 text-xs text-gray-700">Rules: {wizard.rules.map(r =>
                  <span key={r.rule_type} className="mr-1">{r.rule_type.replace(/_/g, " ")}: {r.allowed ? "Allowed" : "Not allowed"}</span>
                )}</div>
                <div className="mt-2 text-sm">
                  <b>Availability (next 2 weeks): </b>
                  {wizard.calendar_data.slice(0, 14).map((d) =>
                    <span key={d.date} className={`inline-block rounded px-1 ${d.is_available ? "bg-green-200" : "bg-red-200"}`}>{d.date.slice(5)} {d.is_available ? "✓" : "×"}</span>
                  )}
                </div>
              </div>
              {is_edit_mode &&
                <div className="mt-4 text-yellow-700 bg-yellow-50 p-2 rounded border border-yellow-200 text-sm">
                  <div>
                  <b>LIMITATION (MVP):</b> You can only edit general info, NOT photos, amenities, or rules for an existing villa. To update those, please contact support.
                  </div>
                </div>
              }
              <div className="mt-6">
                ...
                <span className="ml-3">
                  <Link to="/host/dashboard" className="underline text-gray-500 hover:text-blue-700">
        ...
        <div className="mt-7 text-xs text-gray-500 border-t pt-4">Fields marked * are required.<br />
          {is_edit_mode ? (
            <>Listing edit is limited (MVP): <b>photos, amenities and rules</b> cannot be updated here. Contact support for advanced editing.</>
          ) : (
            <>Your listing will be published and appear to guests after completing all steps and publishing.<br />
              <span className="text-blue-600 underline"><Link to="/host/dashboard">Back to Host Dashboard</Link></span>
            </>
          )}
        </div>
      </div>
