...
type AppStore = {
  ...
  update_message_thread: (thread_id: number, props: Partial<MessageThread>) => void;
      ...
      add_message_thread: (thread) =>
        set((state) => ({
          messaging_threads: {
            ...
            threads: [thread, ...state.messaging_threads.threads.filter((t) => t.thread_id !== thread.thread_id)],
          },
        })),
      ...
      connect_socket: (token) => {
        ...
        socket.on('message_new', (payload) => {
          set((state) => {
            ...
            const idx = newThreads.findIndex((t) => t.thread_id === payload.thread_id);
            if (idx !== -1) {
